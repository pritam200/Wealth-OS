...
@Component
@Order(1)
public class KfintechParser implements EmailParser {

    // Longer/more-specific alternatives listed first — see CamsParser for why (Java regex
    // alternation stops at the first match; "Scheme" alone would match and then fail the
    // trailing colon check when the real label is "Scheme Name:", missing it entirely).
    private static final Pattern FUND   = Pattern.compile(
        "(?:Scheme\\s*Name|Fund\\s*Name|Scheme|Fund)\\s*[:]\\s*([A-Za-z][A-Za-z0-9 &.,\\-()/]+?)\\s*(?:\\r?\\n|\\||$)",
        Pattern.CASE_INSENSITIVE);
    private static final Pattern AMOUNT = Pattern.compile(
        "(?:SIP\\s+Amount|Purchase\\s+Amount|Investment\\s+Amount|Amount\\s+Paid|Net\\s+Amount|Amount|SIP)[:\\s]+(?:Rs\\.?\\s*|₹\\s*)?([\\d,]+\\.?\\d*)",
        Pattern.CASE_INSENSITIVE);
    ...
    public boolean canParse(String from, String subject) {
        return ParserUtil.containsIgnoreCase(from, "kfintech", "karvy", "kfin")
    ...
    public List<ParsedEmail> parse(String from, String subject, String bodyText) {
        ...
        results.add(ParsedEmail.builder()
            ...
            .amount(amount)
            .nav(ParserUtil.parseMoney(ParserUtil.findFirst(text, NAV)))
            .units(ParserUtil.parseMoney(ParserUtil.findFirst(text, UNITS)))
            .tradeDate(tradeDate != null ? tradeDate : LocalDate.now())
            .folio(folio)
            .provider(provider)
            .sourceDescription(String.format("KFintech MF %s: %s ₹%.0f", typeLabel, fundName.trim(), amount))
            .build());
        return results;
    }
