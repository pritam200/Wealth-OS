...
@Component
@Order(1)
public class AngelOneParser implements EmailParser {

    private static final Pattern SYMBOL  = Pattern.compile("(?:Symbol|Scrip(?:Name)?|Stock|Instrument)[:\\s]+([A-Z][A-Z0-9&-]{1,20})", Pattern.CASE_INSENSITIVE);
    ...
    public boolean canParse(String from, String subject) {
        return ParserUtil.containsIgnoreCase(from, "angelone", "angel broking", "angel one")
            && ParserUtil.containsIgnoreCase(subject, "trade confirmation", "contract note", "order", "executed",
    ...
    public List<ParsedEmail> parse(String from, String subject, String bodyText) {
        ...
        if (count == 0) return ContractNoteHelper.parseContractNoteRows(text, subject, "Angel One");
        ...
        for (int i = 0; i < count; i++) {
            ...
            results.add(ParsedEmail.builder()
                ...
                .sourceDescription(String.format("Angel One %s: %s x %d @ ₹%.2f", type == ParsedEmail.Type.TRADE_BUY ? "Buy" : "Sell", sym, qty, price))
                .build());
        }
        return results;
    }
