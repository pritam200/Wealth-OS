...
@Component
@Order(1)
public class ZerodhaParser implements EmailParser {
    ...
    public boolean canParse(String from, String subject) {
        return ParserUtil.containsIgnoreCase(from, "zerodha", "kite")
    ...
    public List<ParsedEmail> parse(String from, String subject, String bodyText) {
        ...
        if (count == 0) return ContractNoteHelper.parseContractNoteRows(text, subject, "Zerodha");
        ...
        for (int i = 0; i < count; i++) {
            ...
            results.add(ParsedEmail.builder()
                ...
                .sourceDescription(String.format("Zerodha %s: %s x %d @ ₹%.2f", type == ParsedEmail.Type.TRADE_BUY ? "Buy" : "Sell", sym, qty, price))
                .build());
        }
        return results;
    }
