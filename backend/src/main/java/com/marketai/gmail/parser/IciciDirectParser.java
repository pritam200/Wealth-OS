...
@Component
@Order(1)
public class IciciDirectParser implements EmailParser {
    ...
    public boolean canParse(String from, String subject) {
        return ParserUtil.containsIgnoreCase(from, "icicidirect")
    ...
    public List<ParsedEmail> parse(String from, String subject, String bodyText) {
        ...
        if (count == 0) return ContractNoteHelper.parseContractNoteRows(text, subject, "ICICIdirect");
        ...
        for (int i = 0; i < count; i++) {
            ...
            results.add(ParsedEmail.builder()
                ...
                .sourceDescription(String.format("ICICIdirect %s: %s x %d @ ₹%.2f", type == ParsedEmail.Type.TRADE_BUY ? "Buy" : "Sell", sym, qty, price))
                .build());
        }
        return results;
    }
